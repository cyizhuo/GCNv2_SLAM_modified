    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    if (getenv("USE_ORB") == nullptr)
    {
        ...
        if (mSensor == System::MONOCULAR) {
            mpIniGCNextractor = new GCNextractor(2 * nFeatures, fScaleFactor, nLevels, fIniThFAST, fMinThFAST);
        }
    }
    else
    {
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET) {
        ...
        {
            // GCN
            mCurrentFrame = Frame(mImGray,timestamp,mpIniGCNextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
